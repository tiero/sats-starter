  ...
  useEffect(() => {
    (async () => {
      ...
      const beneficiaryKeyPair = ECPair.fromPrivateKey(
        Buffer.from("be945b04e35608be9db21b90f34023ffb87ae9c7a1ef125e091cd93f3f553878", "hex"), 
        { network }
      );
      const donorKeyPair = ECPair.fromPrivateKey(
        Buffer.from("e36451789f29cdd2e4dcd44c2c9f8d7cd8c869b862d2a283c0860b9484b5adef", "hex"), 
        { network }
      );
      //console.log(beneficiaryKeyPair.privateKey?.toString('hex'), donorKeyPair.privateKey?.toString('hex'));

      const beneficiary = payments.p2wpkh({ pubkey: beneficiaryKeyPair.publicKey, network });
